use crate::{
    catalog::MetadataStream, error::PdfResult, objects::Dictionary, pdf_enum,
    postscript::PostscriptInterpreter, stream::Stream, Resolve,
};
...
impl Type1FontFile {
    pub fn from_stream(mut stream: Stream, resolver: &mut dyn Resolve) -> PdfResult<Self> {
        ...
        let decoded = &*crate::filter::decode_stream(&stream.stream, &stream.dict, resolver)?;
        println!("{}", String::from_utf8_lossy(decoded));

        // PostscriptInterpreter::new(b"4 3 lt {(TruePart)} {{(FalsePart)} exec} ifelse")
        //     .run()
        //     .unwrap();
        // PostscriptInterpreter::new(b"(a)(b)(c)(d) 3 index")
        //     .run()
        //     .unwrap();
        PostscriptInterpreter::new(decoded).run().unwrap();
