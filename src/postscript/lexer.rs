...
impl<'a> PostScriptLexer<'a> {
    ...
    fn lex_object(&mut self) -> PdfResult<PostScriptObject> {
        self.skip_whitespace();
        ...
        match self.peek_byte() {
            ...
            Some(b'/') => {
                ...
                Ok(PostScriptObject::Name(PostScriptString::from_bytes(name)))
            }
            Some(b'(') => {
                let s = self.lex_string()?.into_bytes();
                Ok(PostScriptObject::String(
                    self.strings.insert(PostScriptString::from_bytes(s)),
                ))
            }
            Some(b'<') => self.lex_gt(),
